...
func (r *RaylibRenderer) UpdateLayout(roots []*render.RenderElement) {
    ...
    r.roots = roots // Store the current roots if they can change per frame, or assume they are stable after PrepareTree

	// Perform layout for all root elements and their descendants
	for _, root := range r.roots { // Use r.roots which should be set by PrepareTree or updated
		if root != nil {
			// PerformLayout is assumed to be a method of r (from renderer_processing.go)
	...
	r.ApplyCustomComponentLayoutAdjustments() // This is a method of r
}
...
func (r *RaylibRenderer) PollEventsAndProcessInteractions() {
	...
	for i := len(r.elements) - 1; i >= 0; i-- {
		...
		if strings.HasPrefix(el.SourceElementName, "tab_") {
			log.Printf("PollEvents Check: Elem '%s' (Idx %d, Type %X), IsInteractive: %t, Visible: %t, Bounds: (X:%.1f Y:%.1f W:%.1f H:%.1f), Mouse: (%.1f, %.1f)",
				el.SourceElementName, el.OriginalIndex, el.Header.Type, el.IsInteractive, el.IsVisible,
				...
				if isCustomInstance && componentID != "" {
					if customHandler, handlerExists := r.customHandlers[componentID]; handlerExists {
						if eventInterface, implementsEvent := customHandler.(render.CustomEventHandler); implementsEvent {
							handled, err := eventInterface.HandleEvent(el, krb.EventTypeClick, r)
							if err != nil {
...
func (r *RaylibRenderer) DrawFrame(roots []*render.RenderElement) {
    r.roots = roots // Store/update roots if they can change per frame
	for _, root := range r.roots { // Use r.roots
		if root != nil {
			r.renderElementRecursiveWithCustomDraw(root, r.scaleFactor)
		}
	}
